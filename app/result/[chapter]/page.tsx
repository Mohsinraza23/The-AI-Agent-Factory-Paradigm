...
export default function ResultPage() {
  ...
  const score = answers.reduce((acc: number, ans, i) => ans === chapter.questions[i].correct ? acc + 1 : acc, 0);
  const skipped = answers.filter((a) => a === null).length;
  const percentage = Math.round((score / chapter.questions.length) * 100);
  const passed = percentage >= 50;

  const grade =
    percentage >= 90 ? { label: "A+", color: "text-indigo-400" } :
    percentage >= 80 ? { label: "A",  color: "text-indigo-400" } :
    percentage >= 70 ? { label: "B",  color: "text-violet-400" } :
    percentage >= 60 ? { label: "C",  color: "text-yellow-400" } :
    percentage >= 50 ? { label: "D",  color: "text-orange-400" } :
    { label: "F", color: "text-red-400" };

  return (
    <main className="min-h-screen bg-[#07080f] px-4 py-8">
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-32 -right-32 w-80 h-80 bg-indigo-500/10 rounded-full blur-3xl animate-orb-1" />
        <div className="absolute bottom-0 -left-32 w-80 h-80 bg-violet-400/10 rounded-full blur-3xl animate-orb-2" />
      </div>
      ...
      <div className="relative z-10 max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6 animate-fade-up">
          <div className="flex items-center gap-2">
            ...
            <span className="text-indigo-300 text-xs font-semibold uppercase tracking-widest">Quiz Complete</span>
          </div>
          ...
          <div className="p-5 sm:p-6">
            <div className="flex items-end justify-between mb-5">
              <div>
                ...
                <p className="animate-pop text-5xl sm:text-6xl font-black text-white leading-none">
                  {score}
                  <span className="text-xl sm:text-2xl text-gray-500 font-semibold ml-1">/ {chapter.questions.length}</span>
                </p>
              </div>
              <div className="text-right">
                <p className="text-gray-500 text-xs uppercase tracking-widest mb-2">Grade</p>
                <p className={`animate-pop text-5xl sm:text-6xl font-black leading-none ${grade.color}`}>{grade.label}</p>
              </div>
            </div>
            <div className="w-full bg-white/5 rounded-full h-3 mb-2 overflow-hidden">
              <div
                className={`h-3 rounded-full transition-all duration-1000 delay-300 ${passed ? "bg-gradient-to-r from-indigo-500 to-violet-400" : "bg-gradient-to-r from-red-500 to-rose-400"}`}
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                ...
                { val: `${percentage}%`,                         label: "Score", icon: "%", color: "text-indigo-400", bg: "hover:bg-indigo-500/10  hover:border-indigo-500/30" },
              ].map((s) => (
                <div key={s.label} className={`bg-white/4 border border-white/8 rounded-2xl p-3 text-center transition-all duration-200 cursor-default ${s.bg}`}>
        ...
        {/* YouTube CTA */}
        <a href="https://www.youtube.com/@CodewithMohsin1" target="_blank" rel="noopener noreferrer"
          className="group block mb-6 animate-fade-up" style={{ animationDelay: "0.1s" }}>
          <div className="relative bg-gradient-to-r from-red-600/20 via-red-500/10 to-red-600/20 hover:from-red-600/30 hover:to-red-600/30 border border-red-500/25 hover:border-red-500/60 rounded-2xl px-4 py-4 transition-all duration-300 hover:scale-[1.01] hover:shadow-lg hover:shadow-red-500/15 overflow-hidden">
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="flex-shrink-0 bg-red-600 group-hover:bg-red-500 text-white text-xs font-black px-4 py-2 rounded-full transition-colors duration-200 shadow-lg shadow-red-600/30">Subscribe</span>
            </div>
          </div>
        </a>
        ...
        <div className="flex items-center gap-3 mb-4 px-1">
          <h2 className="text-indigo-300 font-black text-sm uppercase tracking-widest">Jawabat ka Jaiza</h2>
        ...
        <div className="space-y-3">
          {chapter.questions.map((q, i) => {
            ...
            return (
              <div
                key={q.id}
                className="animate-fade-up bg-[#0d0d1f] border border-white/8 rounded-2xl overflow-hidden hover:border-white/15 transition-all duration-200"
              ...
              >
                ...
                <div className="p-3 space-y-1.5">
                  {q.options.map((option, j) => {
                    ...
                    return (
                      <div key={j} className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl border ${rowStyle} transition-colors duration-150`}>
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Link
            ...
            className="flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 active:bg-white/15 border border-white/10 hover:border-indigo-500/40 text-white hover:text-indigo-300 font-bold py-4 rounded-2xl text-center transition-all duration-200 hover:scale-[1.01]"
          >
          ...
          <Link
            href="/"
            className="flex items-center justify-center gap-2 bg-gradient-to-r from-indigo-500 to-violet-500 hover:from-indigo-400 hover:to-violet-400 active:from-indigo-600 active:to-violet-600 text-white font-bold py-4 rounded-2xl text-center transition-all duration-200 shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/40 hover:scale-[1.01]"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="mt-8 pb-4 text-center">
          <p className="text-gray-700 text-xs">
            Designed &amp; Created by{" "}
            <span className="text-indigo-400 font-bold tracking-wide">✦ Mohsin Raza</span>
          </p>
        </div>
      </div>
    </main>
