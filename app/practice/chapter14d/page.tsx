...
export default function PracticePageD() {
  ...
  function handleSelect(i: number) {
    ...
    setSelected(i);
    setRevealed(true);
    // Only score if correct AND no hint was used on this question
    if (i === question.correct && !hintPenaltyThisQ) {
      setScore((s) => s + 1);
    }
  }
        ...
        {done ? (
          <div className="animate-pop">
            <div className="bg-[#0d0d1f] border border-white/10 rounded-3xl overflow-hidden shadow-2xl shadow-black/40 mb-5">
              <div className={`px-6 py-5 ${percentage >= 50 ? "bg-gradient-to-r from-violet-600 to-fuchsia-500" : "bg-gradient-to-r from-red-700 to-rose-600"}`}>
                <p className="text-white/70 text-xs font-semibold uppercase tracking-widest mb-1">
                  {percentage >= 50 ? "Mubarak Ho!" : "Mehnat Karein!"}
                </p>
                <p className="text-white font-black text-xl">
                  {percentage >= 50 ? "Guided Practice Complete!" : "Dobara Practice Karein"}
                </p>
              </div>
              <div className="p-6">
                ...
                <div className="w-full bg-white/5 rounded-full h-3 mb-2 overflow-hidden">
                  <div
                    className={`h-3 rounded-full ${percentage >= 50 ? "bg-gradient-to-r from-violet-500 to-fuchsia-400" : "bg-gradient-to-r from-red-500 to-rose-400"}`}
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="bg-fuchsia-500/8 border border-fuchsia-500/20 rounded-2xl p-3 text-center">
                    <p className="text-2xl font-black text-fuchsia-400">{hintsUsed}</p>
                    <p className="text-gray-600 text-xs mt-0.5">Hints 💡</p>
                  </div>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button
                ...
                className="flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-violet-500/40 text-white hover:text-violet-300 font-bold py-4 rounded-2xl transition-all duration-200 hover:scale-[1.01]"
              >
              ...
              <Link
                href="/"
                className="flex items-center justify-center gap-2 bg-gradient-to-r from-violet-500 to-fuchsia-500 hover:from-violet-400 hover:to-fuchsia-400 text-white font-bold py-4 rounded-2xl transition-all duration-200 shadow-lg shadow-violet-500/20 hover:scale-[1.01]"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <>
            ...
            {!revealed && (
              <p className="text-center text-gray-700 text-xs mt-2">
                {hintActive ? "Hint dekh liya — ab jawab chunein" : "Hint chahiye? Upar button dabayein (score nahi milega)"}
              </p>
            )}
          </>
        )}
